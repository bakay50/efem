...
<content tag="application">
    <layout:pageResources dependsOn="application">
        ...
        <layout:resource name="/resources/searchTrsferResources"/>
    ...
    <h3 id="title"><g:message code="transferOrder.search.title" default="SEARCH TRANSFERT"/></h3>
    <g:render template="/utils/messages"/>
    <wf:searchForm name="searchForm" action="search" method="get" model="[searchCommand: searchCommand]"
                   template="search/searchCriteria"/>
    <bootstrap:div id="searchResults">
        <g:render template="/utils/search/searchResults"
                  model="[searchCommand: searchCommand,
                          max          : max,
                          domain       : UtilConstants.TRANSFER,
                          resultList   : resultList,
                          totalCount   : totalCount]"/>

    </bootstrap:div>
    <g:if test="${resultList?.size() > 0}">
        <g:render template="/transferOrder/search/exportList" model="[searchCommand: searchCommand]"/>
    </g:if>
